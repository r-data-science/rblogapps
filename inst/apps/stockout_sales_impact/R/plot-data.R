...
#' @importFrom ggtext element_textbox_simple element_markdown geom_richtext geom_textbox
#' @importFrom fs dir_create path
#' @importFrom magick image_read image_resize image_crop image_trim image_background image_convert image_write
...
process_image <- function(url, sku, outdir = "images/products") {
  if (!requireNamespace("magick", quietly = TRUE))
    stop("package 'magick' not available", call. = FALSE)

  img_path <- fs::dir_create(outdir, recurse = TRUE) |>
    fs::path(paste0(sku, ".jpeg"))
...
plot_base_layer <- function(salesDT) {
  ...
  img_path <- tryCatch({
    ...
    msg <- paste0("Unable to process image for sku ", salesDT[1, product_sku])
  ...
  if (!is.null(img_path)) {
    b0 <- b0 +
      ggtext::geom_richtext(
        ggplot2::aes(x = 1, y = 0),
        label = paste0("<img src='", img_path, "' width='125'/>"),
        hjust = -.3,
        vjust = 0,
        color = NA
      ) +
...
plot_period_impact <- function(salesDT, sku, impact_day) {
